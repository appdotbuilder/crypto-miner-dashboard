...
import { db } from '../db';
import { usersTable, balancesTable, miningSessionsTable, transactionsTable } from '../db/schema';
...
describe('withdrawMining', () => {
  ...
  it('should withdraw mining balance to Bitcoin balance', async () => {
    ...
    expect(result.amount).toEqual(0.001);
  ...
  it('should add mining balance to existing Bitcoin balance', async () => {
    // Create test user
    const userResult = await db.insert(usersTable)
      .values({})
      .returning()
    ...
    expect(parseFloat(balances[0].amount)).toEqual(0.007); // 0.005 + 0.002
  });
  ...
  it('should create new Bitcoin balance if none exists', async () => {
    ...
    expect(balances).toHaveLength(1);
    expect(parseFloat(balances[0].amount)).toEqual(0.003);
  });
  ...
  it('should reset mining balance to zero', async () => {
    ...
    // Check mining balance is reset
    const miningSessions = await db.select()
      .from(miningSessionsTable)
      .where(eq(miningSessionsTable.id, miningResult[0].id))
      .execute();
  ...
  it('should save transaction to database', async () => {
    ...
    expect(parseFloat(transactions[0].amount)).toEqual(0.0015);
  ...
  it('should throw error when no mining session exists', async () => {
    ...
    expect(withdrawMining(input)).rejects.toThrow(/no mining session found/i);
  });
  ...
  it('should throw error when mining balance is zero', async () => {
    ...
    expect(withdrawMining(input)).rejects.toThrow(/no mining balance to withdraw/i);
  });
});
